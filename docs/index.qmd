...
viewer_tags <- tags$div(
  ...
  (function(){
    ...
    const cy = cytoscape({
      ...
      layout: { name: 'cose', padding: 30, nodeRepulsion: 9000, idealEdgeLength: 110, animate: false }
    });
    ...
    function applyNodeSizing(){
      ...
      cy.batch(() => {
        cy.nodes().forEach(n => {
          ...
          const degFactor = (boost && maxDeg) ? (0.65 + 0.6 * (deg / maxDeg)) : 1;
          const fontSize = Math.max(7, baseFont * scale * degFactor);
          const pad      = Math.max(4, basePad * scale * degFactor);
    ...
    function relayout(name){
      const commonPad = 30;
      const opts = (name==='concentric') ? { name, minNodeSpacing: 30, padding: commonPad } :
                   (name==='breadthfirst') ? { name, directed: true, padding: commonPad, spacingFactor: 1.2 } :
                   (name==='fcose') ? {
                      name,
                      quality: 'proof',
                      randomize: true,
