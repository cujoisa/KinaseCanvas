...
cantley_pairs <- cantley_filtered %>%
  transmute(Kinase, Kinase_entrez, Substrate, Substrate_entrez, Site, Curated_site = curated_site,
            ...
            Evidence = "Specificity Atlas (high-confidence, co-localized, functional)",
...
stage_vars <- c("Specificity Atlas", "Functional Site?", "Co-localized?", "Filtered Result")
...
p3A_coverage <- ggplot(cov_tbl, aes(x = metric, y = count, fill = metric)) +
  ...
  annotate(
    ...
    label = {
      ...
      inc <- df$`Novel pairs` / (df$`Curated pairs` + df$`Novel pairs`)
