...
edge_tbl <- combined_pairs %>%
  mutate(
    evidence = if_else(Source %in% curated_sources, "Known (curated)", "Novel (Specificity Atlas)")
  ) %>%
  group_by(Kinase, Substrate) %>%
  summarise(
    evidence      = if_else(any(evidence == "Known (curated)"), "Known (curated)", "Novel (Specificity Atlas)"),
...
viewer_tags <- tags$div(
  ...
  tags$div(
    ...
    tags$span(class="chip", tags$span(class="line novel"), "Novel (Specificity Atlas)"),
  ...
  (function(){
    ...
    const cy = cytoscape({
      ...
      style: [
        ...
        { selector: 'edge[evidence = \"Novel (Specificity Atlas)\"]',
