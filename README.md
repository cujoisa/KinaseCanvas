...
The pipeline took under 15 minutes on an Apple M3 Ultra (2.5 minutes in the most recent run). It needs network access only to fetch
...
| `results/unfiltered_cantley_dataset.csv` | Supplementary Table S1: the complete unfiltered prediction matrix (3.6 GB) |
...
Copies of Supplementary Tables S2-S4 and the relaxed tier, named as the manuscript cites them (`paper/Supplementary_Table_S2.csv` to `S4.csv` and `paper/Supplementary_Data_Relaxed_Motif_Tier.csv`), sit in `paper/` for upload with the manuscript. They are not regenerated by the pipeline, so recopy them from `results/` after a rerun. Supplementary Table S1 is too large to upload and is cited from this archive as `results/unfiltered_cantley_dataset.csv`.
